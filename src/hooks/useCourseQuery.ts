...
import type { CoursesParams, Enrollment } from '../types';
import { useCourseStore, type CourseStore } from '../stores/courseStore';
...
export const useCourses = (params?: CoursesParams) => {
  const setCourses = useCourseStore((s: CourseStore) => s.setCourses);
  const setLoading = useCourseStore((s: CourseStore) => s.setLoading);
  const setError = useCourseStore((s: CourseStore) => s.setError);
  const setPagination = useCourseStore((s: CourseStore) => s.setPagination);
...
export const useCourseDetail = (id: string | undefined) => {
  const setSelectedCourse = useCourseStore((s: CourseStore) => s.setSelectedCourse);
  const setLoading = useCourseStore((s: CourseStore) => s.setLoading);
  const setError = useCourseStore((s: CourseStore) => s.setError);
...
export const useMyEnrollments = (status: EnrollmentStatus) => {
  const setEnrollments = useCourseStore((s: CourseStore) => s.setEnrollments);
  const setLoading = useCourseStore((s: CourseStore) => s.setLoading);
  const setError = useCourseStore((s: CourseStore) => s.setError);
